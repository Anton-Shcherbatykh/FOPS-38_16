...
Объявил в файле ```outputs.tf``` один output, содержащий ```external_ip``` для каждой из ВМ
...
В файле ```locals.tf``` описал в одном local-блоке имя каждой ВМ, используя интерполяцию ${..}

![alt text](Pictures/pic010.jpg)

Т.к. в ```variable.tf``` у меня нет переменных с именем ВМ, а всё вынесено в ```vms_platform.tf```, то в ```main.tf``` сослался на созданный ```locals.tf``` и применил изменения.
...
Из-за срочного вызова по основной работе вынужден был отложить выполнение задания и чтобы не тратить впустую ресурсы, удалил всё ранее созданное через terrform destroy. Но с рабочими вопросам удалось быстро разобраться, поэтому продолжил через несколько минут выполнять задание. Не стал восстанавливать предыдущий конфиг, сразу начал менять параметры для выполнения задания 6.
...
Проверил всё с помощью terraform plan

![alt text](Pictures/pic016.jpg)

Затем запустил проект с помощью terraform apply и получил две работающие ВМ
